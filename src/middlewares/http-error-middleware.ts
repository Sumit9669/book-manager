...
const HttpErrorMiddleware = async (
  error: BooksManagerException | Error | ExpressJoiError | any,
  request: Request,
  response: Response,
  next: NextFunction
) => {
  if (error instanceof BooksManagerException) {
    ...
    if (error.code === "ENOTFOUND") {
      const newError = new BooksManagerException(
      ...
      const exception = new BooksManagerException(
        err,
        error.response.data.message,
        error.stack
      );
      ...
      const err = new BooksManagerException(
    ...
    let validationError: BooksManagerException;
    if (error.type === "headers") {
      validationError = new BooksManagerException(
...
const HandleException = (error: any, request: Request, response: Response) => {
  const errorDetails: BooksManagerException = {
